...
sudo -i -u postgres /usr/lib/postgresql/9.4/bin/psql -q -c "DROP ROLE IF EXISTS \"$USER\";"
sudo -i -u postgres /usr/lib/postgresql/9.4/bin/psql -q <<-EOF
...
EOF
    ...
    if [[ ! -z "$EXTENSIONS" ]]; then
        for extension in $EXTENSIONS; do
            ...
            sudo -i -u postgres /usr/lib/postgresql/9.4/bin/psql -q "$DB" -c "CREATE EXTENSION \"$extension\";"
        done
    fi
fi
